// server.js  (ESM)
// package.json needs:  { "type": "module", "scripts": { "start": "node server.js" } }
// Node 18+
//
...
if (!OPENAI_API_KEY) console.warn("[WARN] OPENAI_API_KEY not set (chat/photo will fail).");
...
// Create / migrate schema (idempotent)  **Option B: keep original daily quotas**
async function ensureSchema() {
  await pool.query(`
    ...
    -- Free-tier usage per device (per natural day)  **Option B**
  ...
  // Backfill safe columns for older installs
...
async function resendSend({ to, subject, html, text }) {
  if (!RESEND_API_KEY || !RESEND_FROM) {
    console.warn("[RESEND] Skipped (env incomplete)");
  ...
  try {
    ...
    await pool.query(
      `update users set verify_token=$2, verify_expires=$3, verified=false, updated_at=now()
        where email=$1`,
      [email, until.toISOString() ? token : token, until.toISOString()]
    );
    ...
    try {
      ...
      console.warn("[RESEND] error (ignored for signup):", e?.message || e);
    }
...
app.post("/api/resend-verify", async (req, res) => {
  try {
    ...
    res.status(500).json({ status: "error", message: "Could not send verification email" });
  }
});
...
app.post("/api/forgot-password", async (req, res) => {
  try {
    ...
    const u = await getUserByEmail(email);
    if (!u) return res.json({ status: "ok" }); // don't leak whether user exists
    ...
    await resendSend({
      ...
      html: `<p>Reset your password:</p><p><a href="${link}">Reset password</a></p><p>If you did not request this, ignore this email.</p>`,
    ...
    res.status(500).json({ status: "error", message: "Could not start reset" });
  }
});
...
app.post("/api/reset-password", async (req, res) => {
  try {
    ...
    await pool.query(`update users set reset_token=null, reset_expires=null, updated_at=now() where email=$1`, [email]);
    res.json({ status: "ok" });
  } catch (e) {
...
app.patch("/api/conversations/:id", async (req, res) => {
  ...
  if (!Number.isFinite(id)) return res.status(400).json({ status: "error", message: "Invalid id" });
...
app.post("/api/chat", async (req, res) => {
  try {
    const u = await requireUser(req, res); if (!u) return;
    if (!u.verified) {
      return res.status(403).json({ status: "verify_required", message: "Please verify your email to start chatting." });
    }
