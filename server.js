...
// Small helpers for conditional migrations
...
async function ensureSchema() {
  ...
  // 2) Conversations – create if missing (user_id initially NULLable so we can backfill)
  if (!(await tableExists("conversations"))) {
    await pool.query(`
      create table conversations (
        id            bigserial primary key,
        user_id       bigint,                  -- will be NOT NULL after backfill
        title         text not null,
  ...
  // If conversations exists but user_id column doesn't, add it (NULLable)
  if (!(await columnExists("conversations", "user_id"))) {
    await pool.query(`alter table conversations add column user_id bigint;`);
  }

  // 2a) Backfill user_id from older schemas:
  // - If an old column "user_email" exists, map it to users.id
  if (await columnExists("conversations", "user_email")) {
    await pool.query(`
    ...
    `);
    // optional: drop old column after backfill
    await pool.query(`alter table conversations drop column user_email;`);
  }

  // - If anything still NULL (no way to map), safest is to delete the orphans (they're unusable anyway)
  await pool.query(`delete from conversations where user_id is null;`);

  // 2b) Enforce NOT NULL + FK now that no NULLs remain
  // Add FK if missing
  if (!(await constraintExists("conversations", "conversations_user_fk"))) {
    await pool.query(
      `alter table conversations add constraint conversations_user_fk
         foreign key (user_id) references users(id) on delete cascade`
    );
  }
  // Set NOT NULL (safe because we just deleted/filled NULLs)
  await pool.query(`alter table conversations alter column user_id set not null;`);
...
// ---------- Chat ----------
async function remainingMessageIfLimited(res, gate) {
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (isMath) {
      ...
      if (!gate.allowed) return remainingMessageIfLimited(res, gate);
    }
