...
async function ensureSchema() {
  // Idempotent CREATEs — no destructive alters.
  await pool.query(`
  ...
  try {
    ...
    if (data?.status && data?.data?.status === "success") {
      ...
      if (customerEmail) {
        ...
        if (label !== "ONE_TIME") {
          await pool.query(`update users set plan=$2, updated_at=now() where email=$1`, [
            customerEmail,
            label,
          ]);
        }
...
app.post("/api/conversations/:id/share", async (req, res) => {
  ...
  if (!u) return;
  const id = Number(req.params.id);
  const own = await pool.query(`select id from conversations where id=$1 and user_id=$2`, [
    id,
    u.id,
  ]);
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    res.status(200).json({ response: "Sorry, I couldn’t complete that. Please try again." });
  }
});

// ---------- Photo Solve (authenticated, chat page) ----------
app.post("/api/photo-solve", upload.single("image"), async (req, res) => {
  try {
    ...
    const system =
      "You are Math GPT. Read the problem from the image and solve it step-by-step. Explain clearly using LaTeX where helpful.";
            ...
            content: [
              {
                type: "text",
                text:
                  (attempt ? `Note from user: ${attempt}\n` : "") +
                  "Solve this problem step-by-step. Use LaTeX for math.",
              },
    ...
    res.status(200).json({ response: "Photo solve failed. Please try another image." });
  }
});

// ---------- Home Page Free Photo Solve (NO login, 2/day via cookie) ----------
const HOME_FREE_LIMIT = 2;
const HOME_COOKIE = "hps"; // home photo state
...
function readHomeState(req) {
  try {
    const raw = req.cookies?.[HOME_COOKIE];
    if (!raw) return null;
...
function writeHomeState(res, obj) {
  // Not httpOnly so front-end can reflect remaining count if needed; keep secure/samesite similar.
  res.cookie(HOME_COOKIE, JSON.stringify(obj), {
    secure: true,
    sameSite: "Lax",
    path: "/",
    maxAge: 3 * 24 * 60 * 60 * 1000, // keep a few days
  });
}

app.post("/api/home-photo-solve", upload.single("image"), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ status: "error", message: "image required" });

    // Check & update daily usage
    const state = readHomeState(req) || {};
    const today = dayKey();
    const used = state.day === today ? Number(state.used || 0) : 0;

    if (used >= HOME_FREE_LIMIT) {
      return res.json({
        status: "limit",
        message: "You've used your 2 free photo solves for today.",
        remaining: 0,
      });
    }
    ...
    if (!okTypes.includes(req.file.mimetype)) {
      return res.json({ status: "error", message: "Use PNG, JPG, or WEBP image." });
    }
    if (req.file.size > 6 * 1024 * 1024) {
      return res.json({ status: "error", message: "Image larger than 6MB." });
    }

    const note = (req.body?.attempt || "").toString().slice(0, 500);
    const mime = req.file.mimetype || "image/png";
    const b64 = req.file.buffer.toString("base64");
    const dataUrl = `data:${mime};base64,${b64}`;
    ...
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      console.error("home-photo-solve openai error", r.status, t);
      return res.json({
        status: "error",
        message: "Could not solve the photo. Please try again.",
      });
    }
    const data = await r.json();
    const answer = data?.choices?.[0]?.message?.content || "No result";
    ...
    const newState = { day: today, used: used + 1 };
    writeHomeState(res, newState);

    res.json({
      status: "ok",
      response: answer,
      remaining: Math.max(0, HOME_FREE_LIMIT - (used + 1)),
    });
  } catch (e) {
    console.error("home-photo-solve", e);
    res.json({
      status: "error",
      message: "Photo solve failed. Please try again.",
    });
  }
});
